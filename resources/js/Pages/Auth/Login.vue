  ...
  <div class="min-h-[calc(100vh-10rem)] flex items-center justify-center px-4 py-16">
    <div class="w-full max-w-md">
      ...
      <div class="text-center mb-8">
        <h1 class="text-3xl font-black text-white mb-2">다시 전장으로</h1>
        <p class="text-slate-400 text-sm">로그인하여 커뮤니티에 참여하세요</p>
      </div>
      ...
      <div class="bg-slate-900 border border-slate-800 rounded-2xl p-8">
        ...
        <div class="flex items-center gap-4 mb-6">
          ...
          <span class="text-slate-500 text-xs">또는 이메일로 로그인</span>
          <div class="flex-1 h-px bg-slate-700"></div>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-4">
          ...
          <div>
            <div class="flex items-center justify-between mb-1.5">
              <label class="text-sm font-medium text-slate-300">비밀번호</label>
              <Link v-if="canResetPassword" href="/forgot-password" class="text-xs text-violet-400 hover:text-violet-300 transition-colors">
                비밀번호 찾기
              </Link>
            </div>
            <input
          ...
          <div class="flex items-center gap-2">
            ...
            />
            <label for="remember" class="text-sm text-slate-400 cursor-pointer">로그인 상태 유지</label>
          </div>
      ...
      <p class="text-center text-slate-500 text-sm mt-6">
        아직 계정이 없으신가요?
        <Link href="/register" class="text-violet-400 hover:text-violet-300 font-semibold transition-colors ml-1">
          회원가입
        </Link>
      </p>
    </div>
