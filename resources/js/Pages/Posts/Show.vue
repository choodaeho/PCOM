...
const props = defineProps({
  post: Object,
  // post: { id, title, content, user, faction, created_at, views, vote_count, user_vote, board, comments }
  board: Object,
  comments: Array,
...
// Vote
const voteState = ref(props.userVote ?? null)
const voteCount = ref(props.post.vote_count ?? 0)
...
const replies = (parentId) => (props.comments ?? []).filter(c => c.parent_id === parentId)
</script>
...
<template>
  <div class="max-w-3xl mx-auto px-4 py-10">
    ...
    <div class="flex items-center gap-2 text-sm text-slate-500 mb-6">
      ...
      <span>/</span>
      <span class="text-slate-400 truncate max-w-xs">{{ post.title }}</span>
    </div>
        ...
        <div class="flex items-center justify-between flex-wrap gap-3">
          <div class="flex items-center gap-3">
            ...
            <div class="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-300">
              {{ post.is_anonymous ? '?' : (post.user?.name?.[0]?.toUpperCase() ?? '?') }}
            </div>
            <div>
              <div class="flex items-center gap-2">
                <span class="text-sm font-semibold text-slate-200">
                  {{ post.is_anonymous ? '익명' : (post.user?.name ?? '알 수 없음') }}
                </span>
                <FactionBadge
      ...
      <div class="px-6 py-4 border-t border-slate-800 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          <template v-if="user && user.id === post.user_id">
            <Link :href="`/posts/${post.id}/edit`" class="text-xs text-slate-500 hover:text-slate-300 transition-colors px-2 py-1 rounded">
              수정
            </Link>
            <Link :href="`/posts/${post.id}`" method="delete" as="button" class="text-xs text-red-500 hover:text-red-400 transition-colors px-2 py-1 rounded">
              삭제
            </Link>
          </template>
        ...
        <div v-if="showReport" class="px-6 pb-4 border-t border-slate-800 pt-4">
          <form @submit.prevent="submitReport">
            ...
            <div class="flex gap-2 mt-3">
              <button type="submit" :disabled="reportForm.processing" class="bg-red-600 hover:bg-red-500 text-white text-xs font-semibold px-4 py-2 rounded-lg transition-colors disabled:opacity-50">
                신고 제출
              </button>
              <button type="button" @click="showReport = false" class="bg-slate-700 hover:bg-slate-600 text-slate-300 text-xs font-medium px-4 py-2 rounded-lg transition-colors">
                취소
              </button>
            </div>
          </form>
    ...
    <section>
      ...
      <div v-else class="bg-slate-900 border border-slate-800 rounded-2xl p-5 mb-6 text-center">
        ...
        <Link href="/login" class="text-violet-400 hover:text-violet-300 text-sm font-medium transition-colors">로그인하기 →</Link>
      </div>
            ...
            <div class="flex items-start justify-between mb-3">
              <div class="flex items-center gap-2.5">
                <div class="w-7 h-7 rounded-full bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-300 flex-shrink-0">
                  {{ comment.is_anonymous ? '?' : (comment.user?.name?.[0]?.toUpperCase() ?? '?') }}
                </div>
                <div>
                  <div class="flex items-center gap-2">
                    <span class="text-sm font-semibold text-slate-200">
                    ...
                    <FactionBadge
                      v-if="comment.faction && !comment.is_anonymous"
                      :type="comment.faction"
                      :label="comment.faction_label"
                      :emoji="comment.faction_emoji"
                    />
                  </div>
            ...
            >
              <div class="flex items-center gap-2 mb-2">
                ...
                <div class="w-6 h-6 rounded-full bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-300 flex-shrink-0">
                  {{ reply.is_anonymous ? '?' : (reply.user?.name?.[0]?.toUpperCase() ?? '?') }}
                </div>
                <span class="text-xs font-semibold text-slate-300">
                  {{ reply.is_anonymous ? '익명' : (reply.user?.name ?? '알 수 없음') }}
                </span>
                <FactionBadge
                  v-if="reply.faction && !reply.is_anonymous"
                  :type="reply.faction"
                  :label="reply.faction_label"
                  :emoji="reply.faction_emoji"
                />
