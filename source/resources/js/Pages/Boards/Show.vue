...
<template>
    <div class="max-w-5xl mx-auto px-4 py-10">
                ...
                <div class="flex-shrink-0 mt-1">
                    ...
                    <div v-else class="group relative">
                        <Link
                            ...
                            class="flex items-center gap-2 bg-gray-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400 font-semibold px-4 py-2.5 rounded-xl text-sm cursor-not-allowed opacity-60"
                        >
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                            </svg>
                        ...
                        <div class="absolute bottom-full right-0 mb-2 w-52 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-xs rounded-lg px-3 py-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 text-center">
                            {{ writeBlockReason }}
                        </div>
                    </div>
                </div>
        ...
        <div class="flex items-center gap-2 mb-4 text-xs text-slate-400 dark:text-slate-500">
            ...
            총 {{ posts.meta?.total ?? posts.total ?? 0 }}개의 게시글
        </div>
        ...
        <div class="space-y-2 mb-8">
            <div
                ...
                class="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 border-dashed rounded-2xl py-16 text-center"
            >
                <p class="text-4xl mb-3">📭</p>
                <p class="text-slate-500 dark:text-slate-400">아직 게시글이 없습니다.</p>
                <Link
                    v-if="canWrite"
                    :href="`/boards/${board.slug}/posts/create`"
                    class="inline-flex mt-4 text-violet-500 dark:text-violet-400 hover:text-violet-300 text-sm font-medium transition-colors"
                >
                    첫 번째 글을 작성해보세요 →
                </Link>
            </div>
        ...
        <!-- Pagination -->
        <Pagination v-if="(posts.meta?.last_page ?? posts.last_page ?? 1) > 1" :links="posts.links" />
    </div>
</template>
