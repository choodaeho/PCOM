...
const monthlyRows = (() => {
    ...
    Object.entries(props.data ?? {}).forEach(([faction, rows]) => {
        rows.forEach(row => {
            ...
            map[key][faction] = row.raw_score ?? 0
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
                ...
                <div class="flex gap-1">
                    <button
                        ...
                        :class="[
                            ...
                            selectedMonths === m ? 'bg-violet-600 text-white' : 'bg-gray-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-gray-200 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white'
                        ]"
                    >{{ m }}개월</button>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 overflow-hidden mb-6">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-800">
                <h2 class="text-slate-900 dark:text-white font-semibold">월별 집계</h2>
            </div>

            <div v-if="monthlyRows.length === 0" class="text-center text-slate-400 dark:text-slate-500 py-16">
                집계된 데이터가 없습니다.
            </div>
            ...
            <table v-else class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-slate-800">
                    <tr v-for="row in [...monthlyRows].reverse()" :key="row.month" class="hover:bg-gray-100/30 dark:hover:bg-slate-800/30">
                        <td class="px-6 py-3 text-slate-600 dark:text-slate-300 font-medium">{{ row.month }}</td>
                        <td v-for="(cfg, f) in factionConfig" :key="f" class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <span :class="['font-bold', cfg.textClass]">{{ (row[f] ?? 0).toFixed(1) }}</span>
                                <div class="flex-1 h-1.5 bg-gray-200 dark:bg-slate-700 rounded-full overflow-hidden" style="max-width:60px">
